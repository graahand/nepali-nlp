...
def evaluate_sample(args: argparse.Namespace) -> dict:
    ...
    def update_numeral(stat_key: str, span: str) -> None:
        token_count = len(tokenizer.encode(span, add_special_tokens=False))
...
def write_csv_reports(result: dict, output_dir: str) -> None:
    ...
    subset_path = os.path.join(data_dir, "metrics_summary.csv")
    ...
    numeral_path = os.path.join(data_dir, "numeral_metrics.csv")
    ...
    doc_path = os.path.join(data_dir, "doc_tw_summary.csv")
...
def write_markdown_report(result: dict, output_dir: str) -> None:
    ...
    report_path = os.path.join(report_dir, "universalml_iriis_report.md")
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# UniversalML Tokenizer Evaluation on IRIIS Corpus (Sample)\n\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate UniversalML tokenizer on a sampled IRIIS corpus subset."
    )
    ...
    write_morphology_word_lists(result, args.output_dir, safe_slug(args.model_id))
