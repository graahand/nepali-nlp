...
def write_csv_reports(result: dict, output_dir: str) -> None:
    ...
    subset_path = os.path.join(data_dir, "metrics_summary_qwen.csv")
    ...
    numeral_path = os.path.join(data_dir, "numeral_metrics_qwen.csv")
    ...
    doc_path = os.path.join(data_dir, "doc_tw_summary_qwen.csv")
...
def write_markdown_report(result: dict, output_dir: str) -> None:
    ...
    report_path = os.path.join(report_dir, "qwen_iriis_report.md")
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Qwen Tokenizer Evaluation on IRIIS Corpus (Sample)\n\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate Qwen tokenizer on a sampled IRIIS corpus subset."
    )
