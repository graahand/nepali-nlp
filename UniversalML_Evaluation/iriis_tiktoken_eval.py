...
def encode_tokens(encoding: tiktoken.Encoding, text: str) -> tuple[list[int], list[str]]:
...
def evaluate_sample(args: argparse.Namespace) -> dict:
    ...
    def update_numeral(stat_key: str, span: str) -> None:
        token_ids = encoding.encode(span)
    ...
    for row in dataset:
        ...
        for word in take_words:
            token_ids, tokens = encode_tokens(encoding, word)
            token_count = len(token_ids)
...
def write_csv_reports(result: dict, output_dir: str) -> None:
    ...
    subset_path = os.path.join(data_dir, "metrics_summary_tiktoken.csv")
    ...
    numeral_path = os.path.join(data_dir, "numeral_metrics_tiktoken.csv")
    ...
    doc_path = os.path.join(data_dir, "doc_tw_summary_tiktoken.csv")
...
def write_markdown_report(result: dict, output_dir: str) -> None:
    ...
    report_path = os.path.join(report_dir, "tiktoken_iriis_report.md")
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# TikToken Evaluation on IRIIS Corpus (Sample)\n\n")
        ...
        if result["model"]:
            f.write(f"- Model encoding: {result['model']}\n")
